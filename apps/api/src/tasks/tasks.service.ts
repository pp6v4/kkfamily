...
import { NotificationScheduleService } from '../notifications/notification-schedule.service';
...
export class TasksService {
  constructor(private readonly prisma:PrismaService,private readonly access:AccessService,private readonly notifications:NotificationScheduleService){}
  ...
  async create(userId:string,householdId:string,dto:CreateTaskDto){return{data:await serializable(this.prisma,async tx=>{
    ...
    await this.notifications.syncTask(tx,task);await this.audit(tx,householdId,member.id,'TASK_CREATE',task.id,{type:task.type,assigneeMembershipId:assigneeId});return this.taskOrThrow(tx,householdId,task.id,true);
  })};}
  ...
  async update(userId:string,householdId:string,taskId:string,dto:UpdateTaskDto){return{data:await serializable(this.prisma,async tx=>{
    ...
    await tx.taskHistory.create({data:{taskId,actorMembershipId:member.id,comment:'更新待办内容'}});const updated=await this.taskOrThrow(tx,householdId,taskId);await this.notifications.syncTask(tx,updated);await this.audit(tx,householdId,member.id,'TASK_UPDATE',taskId,{fromVersion:task.version,toVersion:task.version+1});return this.taskOrThrow(tx,householdId,taskId,true);
  })};}
  ...
  async changeStatus(userId:string,householdId:string,taskId:string,dto:UpdateTaskStatusDto){return{data:await serializable(this.prisma,async tx=>{
    ...
    await tx.taskHistory.create({data:{taskId,actorMembershipId:member.id,fromStatus:task.status,toStatus:dto.status,comment:dto.reason?.trim()||null}});const updated=await this.taskOrThrow(tx,householdId,taskId);await this.notifications.syncTask(tx,updated);await this.audit(tx,householdId,member.id,'TASK_STATUS',taskId,{from:task.status,to:dto.status});return this.taskOrThrow(tx,householdId,taskId,true);
  })};}
