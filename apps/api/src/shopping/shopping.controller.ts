...
export class ShoppingController {
  ...
  updateItem(@CurrentUser() user: RequestUser, @Headers('x-household-id') householdId: string, @Param('id') itemId: string, @Body() dto: UpdateShoppingItemDto) {
    return this.shoppingService.updateItem(user.userId, householdId, itemId, dto);
  }
  ...
  @Post('items/:id/repeat')
  repeat(@CurrentUser() user: RequestUser, @Headers('x-household-id') house: string, @Param('id') id: string, @Body() dto: RepeatShoppingItemDto) {
    return this.shoppingService.repeat(user.userId, house, id, dto);
  }
}
