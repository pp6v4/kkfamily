...
export interface TripMember { membershipId: string; canEdit: boolean; photoAdd: boolean; tripRole: 'OWNER' | 'MEMBER'; status: 'ACTIVE' | 'HISTORY' | 'REVOKED'; version: number; membership: { id: string; user: { id: string; nickname: string | null; avatarUrl: string | null } } }
...
export interface PackingTemplateItem { id: string; version: number; name: string; defaultQuantity: string | number | null; unit: string | null; note: string | null; sortOrder: number }
export interface PackingTemplate { id: string; version: number; createdById: string; name: string; description: string | null; archived: boolean; items: PackingTemplateItem[] }
...
export function updateRecipe(recipeId: string, input: { expectedVersion: number; name: string; categoryId?: string | null; ingredients: Array<{ name: string; quantity?: number; unit: string; optional?: boolean }>; seasonings: string[]; steps: string[] }) { return request<Recipe>(`/recipes/${recipeId}`, 'PATCH', input); }
...
export function updateTripMember(tripId: string, member: TripMember, input: { canEdit?: boolean; photoAdd?: boolean; tripRole?: TripMember['tripRole']; status?: 'ACTIVE' | 'REVOKED'; clearResponsibilities?: boolean }) { return request<Trip>(`/trips/${tripId}/members/${member.membershipId}`, 'PATCH', { expectedVersion: member.version, ...input }); }
...
export function updateTripPreparationGroup(tripId: string, group: TripPreparationGroup, name: string, membershipIds: string[]) { return request<TripPreparationGroup>(`/trips/${tripId}/preparation-groups/${group.id}`, 'PATCH', { expectedVersion: group.version, name, membershipIds }); }
...
export function createPackingTemplate(input: { name: string; description?: string | null; items: Array<{ name: string; quantity?: number | null; unit?: string | null; note?: string | null; sortOrder?: number }> }) { return request<PackingTemplate>('/packing-templates', 'POST', input); }
export function updatePackingTemplate(template: Pick<PackingTemplate, 'id' | 'version'>, input: { name?: string; description?: string | null; archived?: boolean; items?: Array<{ id?: string; name: string; quantity?: number | null; unit?: string | null; note?: string | null; sortOrder?: number }> }) { return request<PackingTemplate>(`/packing-templates/${template.id}`, 'PATCH', { expectedVersion: template.version, ...input }); }
