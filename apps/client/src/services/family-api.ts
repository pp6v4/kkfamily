...
export interface Trip { id: string; version: number; title: string; status: 'PLANNING' | 'PENDING' | 'DEPARTING' | 'COMPLETED' | 'CANCELLED'; startsAt: string; endsAt: string | null; destination: string | null; members: TripMember[]; preparationGroups: TripPreparationGroup[]; _count?: { packingItems: number } }
