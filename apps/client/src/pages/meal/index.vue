...
async function loadPage() {
  ...
  loading.value=true; recipes.value=[]; recipeCategories.value=[]; meal.value=undefined; comparison.value=[]; session.value=undefined; errorText.value='';
  try { session.value=await refreshAccess(); await Promise.all([canAccess(session.value,'recipes')?Promise.all([listRecipeCategories(),listRecipes()]).then(async ([groups,r])=>{recipeCategories.value=groups;recipes.value=r;coverUrls.value={};await Promise.all(r.filter(x=>x.coverAssetId).map(async x=>{try{const media=await getMediaReadUrl(x.coverAssetId!);coverUrls.value[x.id]=publicMediaUrl(media.path);}catch{}}));}):Promise.resolve(),loadMealFromDatabase()]); }
...
async function saveCategory() {
  ...
  loading.value=true;errorText.value='';
  try{const created=await createRecipeCategory(value,recipeCategories.value.length);recipeCategories.value=[...recipeCategories.value,created];categoryName.value='';uni.showToast({title:'分类已添加',icon:'success'});}catch(error){fail(error);}finally{loading.value=false;}
}
function startCategoryEdit(item:RecipeCategory) { categoryEditing.value=item;categoryEditName.value=item.name;categoryEditOrder.value=String(item.sortOrder); }
function cancelCategoryEdit() { categoryEditing.value=undefined;categoryEditName.value='';categoryEditOrder.value=''; }
async function saveCategoryEdit() {
  const current=categoryEditing.value;if(!current||loading.value)return;
  const name=categoryEditName.value.trim(),sortOrder=Number(categoryEditOrder.value);
  if(!name){fail(new Error('请输入分类名称'));return;}if(!Number.isInteger(sortOrder)||sortOrder<0||sortOrder>999){fail(new Error('排序请输入0到999的整数'));return;}
  loading.value=true;errorText.value='';
  try{const updated=await updateRecipeCategory(current,{name,sortOrder});recipeCategories.value=recipeCategories.value.map(item=>item.id===updated.id?updated:item).sort((a,b)=>a.sortOrder-b.sortOrder||a.name.localeCompare(b.name,'zh-CN'));cancelCategoryEdit();uni.showToast({title:'分类已更新',icon:'success'});}catch(error){fail(error);}finally{loading.value=false;}
}
async function applyCategoryArchive(item:RecipeCategory) {
  if(loading.value)return;loading.value=true;errorText.value='';
  try{await archiveRecipeCategory(item);recipeCategories.value=recipeCategories.value.filter(group=>group.id!==item.id);if(category.value===item.name)category.value='全部';if(categoryEditing.value?.id===item.id)cancelCategoryEdit();uni.showToast({title:'分类已归档',icon:'none'});}catch(error){fail(error);}finally{loading.value=false;}
}
function confirmCategoryArchive(item:RecipeCategory) { uni.showModal({title:'归档这个分类？',content:'分类会从新菜谱选项中隐藏；已有菜谱仍保留原分类名称。',success:result=>{if(result.confirm)void applyCategoryArchive(item);}}); }
async function applyRecipeStatus(recipe:Recipe,status:Recipe['status']) {
  if(loading.value)return;loading.value=true;errorText.value='';
  try{const updated=await updateRecipeStatus(recipe,status);recipes.value=recipes.value.map(item=>item.id===updated.id?updated:item);uni.showToast({title:status==='PUBLISHED'?'菜谱已发布':status==='ARCHIVED'?'菜谱已下架':'已恢复为草稿',icon:'none'});}catch(error){fail(error);}finally{loading.value=false;}
}
function changeRecipeStatus(recipe:Recipe,status:Recipe['status']) {
  ...
  uni.showModal({title:'下架这道菜？',content:'下架后不能再加入新餐点；已经确认的历史菜单仍会保留。',success:result=>{if(result.confirm)void applyRecipeStatus(recipe,status);}});
}
...
<template>
  <view class="page">
    ...
    <view v-else-if="active==='recipes'">
      ...
      <view v-if="canAccess(session,'recipes','MANAGE')" class="form-box"><text class="block-title plain-title">菜谱分类</text><view class="inline-form"><input v-model="categoryName" maxlength="30" class="entry category-entry" placeholder="如主食、炒菜、海鲜" /><text class="small-action" @tap="saveCategory">添加分类</text></view><view v-for="item in recipeCategories" :key="item.id" class="category-manage-row"><view><text>{{item.name}}</text><text class="material">排序 {{item.sortOrder}} · 第{{item.version}}版</text></view><view><text class="small-action" @tap="startCategoryEdit(item)">编辑</text><text class="archive-link" @tap="confirmCategoryArchive(item)">归档</text></view></view><view v-if="categoryEditing" class="category-editor"><input v-model="categoryEditName" maxlength="30" class="entry" placeholder="分类名称" /><input v-model="categoryEditOrder" type="number" class="entry order-entry" placeholder="排序0-999" /><view class="inline-form"><text class="small-action" @tap="saveCategoryEdit">保存修改</text><text class="archive-link" @tap="cancelCategoryEdit">取消</text></view></view></view>
