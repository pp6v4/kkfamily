...
const packingGroupIndex = computed(()=>Math.max(0,packingGroupOptions.value.findIndex(option=>option.id===packingGroupFilter.value)));
const packingPersonIndex = computed(()=>Math.max(0,packingPersonOptions.value.findIndex(option=>option.id===packingPersonFilter.value)));
const packingStateIndex = computed(()=>Math.max(0,packingStateOptions.findIndex(option=>option.id===packingStateFilter.value)));
...
watch(selectedTripId,resetPackingFilters,{flush:'sync'});
watch(session,resetPackingFilters,{flush:'sync'});
  ...
  <view class="page">
    ...
    <view v-else-if="active === 'trips' && selectedTrip">
      ...
      <view v-if="packingItems.length" class="packing-actions">
        <picker :range="packingGroupOptions" range-key="name" :value="packingGroupIndex" @change="selectPackingFilter('group',$event.detail.value)"><text class="action">{{packingGroupOptions.find(option=>option.id===packingGroupFilter)?.name || '准备组已变化'}} ▾</text></picker>
        <picker :range="packingPersonOptions" range-key="name" :value="packingPersonIndex" @change="selectPackingFilter('person',$event.detail.value)"><text class="action">{{packingPersonOptions.find(option=>option.id===packingPersonFilter)?.name || '负责人已变化'}} ▾</text></picker>
        <picker :range="packingStateOptions" range-key="name" :value="packingStateIndex" @change="selectPackingFilter('state',$event.detail.value)"><text class="action">{{packingStateOptions.find(option=>option.id===packingStateFilter)?.name}} ▾</text></picker>
