...
test('Older forbidden access result cannot clear the newest successful authorization',async()=>{
 const wait=deferred();let reads=0;const f=setup(()=>++reads===1?wait.promise:Promise.resolve(access));f.session.rememberSession(family);const old=f.session.refreshAccess().catch(e=>e);await tick();await f.session.refreshAccess();const original=new ApiError('old-forbidden',403);wait.reject(original);const result=await old;assert.equal(result.cause,original);assert.equal(Object.keys(result).includes('cause'),false);assert.equal(f.session.getStoredSession().permissionVersion,2);
});
