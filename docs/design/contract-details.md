...
V：源码已包含登录、轮换刷新会话、家庭、菜谱、餐点、库存、购物、日历、行程、行李、待办、收藏、档案、看板和站内通知模块。目标契约中仍未联调的外部能力及未执行的数据库验收，不能因源码存在而标记为通过。
...
E/V：登录使用wx.login的code；服务端换取微信身份后签发15分钟访问令牌。刷新采用30天轮换的随机令牌，数据库只存哈希，重复使用旧刷新令牌撤销整个会话族；退出撤销当前会话。客户端遇到访问令牌401时共享一次续期并重试原请求一次；创建/加入家庭这类尚无家庭上下文的请求也使用同一续期锁，并把新访问令牌写入后续家庭上下文。openid/session_key不返回为业务授权凭证，不写日志。微信请求设置10秒超时并返回502；code有效性不确定时不盲目重复同一个code。

E：平台账号、家庭成员、行程成员分层。本人可设置1～30字、去除首尾空格后的家庭显示名，用于点餐人、负责人和同行成员展示；昵称不作为搜索、邀请或授权依据。一个User可有多个Membership，但这只是架构可扩展性，不要求一期做完整多家庭切换运营系统。角色默认权限取并集；成员显式覆盖按模块替换默认值；effect=DENY优先于任何角色。VIEW<EDIT<MANAGE只在同一模块内比较，禁止跨模块推导。每次请求读取当前成员状态和权限版本，停用后立即拒绝；不依赖JWT里旧角色。
...
2026-09-10用户调整：微信订阅通道延期，当前一期保留站内Inbox；不要求现在选模板，不继续开发/联调发送，不开放主动订阅入口。以下微信渠道设计留作后续恢复依据，不作为当前必须完成项，详见[确认记录](decisions-20260910.md)。
