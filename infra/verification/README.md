...
执行顺序：Docker/Compose 可用性 → Compose 配置校验 → API build阶段镜像 → 全部迁移deploy → 迁移status → 数据库与Prisma模型的结构差异检查 → 通知worker单元测试 → HTTP集成测试。启动迁移前核对数据库用户名、地址、端口、库名和schema，任何一项不符即退出。构建失败不会启动测试容器；结构不一致或测试失败会通过退出码返回。
